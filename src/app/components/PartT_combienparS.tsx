  ...
  async function onHandleFormSubmit(data:TFormValues)
{
  ...
  const SalaireM = data.partTSalPS;
  const EmailForUpdate =  data.email;

    const { data: insertedData, error } = await supabase
    .from('Leads')
    .update({PartTSPS:SalaireM})
...
<div className="text-center flex justify-space-between py-2">
  ...
  <div className="text-center mt-6 flex justify-evenly ">
    <button type="button" onClick={() => Back(3)} className="text-l text-white bg-sky-600 rounded-2xl hover:bg-blue-600 text-white font-semibold py-2 px-4">Back</button>
