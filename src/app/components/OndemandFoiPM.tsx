...
type TFormValues = {
  ...
  const [selectedCity, setSelectedCity] = useState<string>("1 fois par mois");
  const fois = [
    { nombre: 'une fois par mois' ,key:1},
    { nombre: '2 fois par mois',key:2 },
    { nombre: '3 fois par mois',key:3 },
    { nombre: '4 fois par mois',key:4 },
    { nombre: '5 fois par mois',key:5 },
  ]
  ...
  async function onHandleFormSubmit(data:TFormValues)
{
  ...
  .from('Leads')
  .update({ OndemandFoiPM: selected.key })
...
<div className="rounded-full h-32 w-32 overflow-hidden">
  ...
  <div className="fixed mb-12 w-62">
      <Listbox value={selected}  {...register('combChaqF')} onChange={setSelected} >
  ...
  <div className="text-center mt-6 flex justify-evenly ">
    <button type="button" onClick={() => Back(12)} className="text-l  text-white bg-sky-600 rounded-2xl hover:bg-blue-600 font-semibold py-2 px-4">Back</button>
